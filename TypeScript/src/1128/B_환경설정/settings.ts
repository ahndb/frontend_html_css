...
: NPM을 이용하면 자바스크립트 라이브러리를 쉽게설치하고 관리
...
1. npm init
  ...
  : -y 옵션을 추가
    질문 없이 기본값으로 packgae.json 파일을 생성
...
: package.json
  ...
  - 프로젝트에서 사용하는 패키지의 모곩과 버전 정보를 관리
  - 프로젝트ㅡ의 시작점을 지정 | 스크립트를 정의 등의 기능 수행
...
devDependecies: 개발 과정에만 필요한 패키지들의 목록
dependencies: 프로젝트 실행에 필요한 패키지들의 목록

? tsconfing.json 생성
: 타입스크립트 프로젝트의 설정을 저장하는 파일
: 파입스크립트 컴파일러가 프로젝트를 컴파일하는 방법을 지정
