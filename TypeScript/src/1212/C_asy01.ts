...
function fetchData(callback: (data: any) => void) {
  ...
  /* 
  fetch(url - 요청을 보낼 수조)
    .then(response => response.json()); // JSON형태로 응답을 처리, 존재하는것을 가져오는 것이기 때문에 Read같음.
    .then(data => console.log(data)); // 데이터 처리 및 사용
    .catch(error => console.log(error)); // fetch에서 일어나는 오류
  */
