...
//? 문서객체.innerHTML
// : 입력된 문자열을 HTML 형식으로 넣음

document.addEventListener('DOMContentLoaded',() => {
  const a = document.querySelector('#textContent')
  const b = document.querySelector('#innerHTML')

  a.textContent = '<h2>textContent 속성</h2>'; //문자열 다 출력됨
  b.innerHTML = '<h2>innerHTML 속성</h2>'; //요소는 형식으로 남고 문자만 출력
})
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  rects.forEach((rect, index) => {
    ...
    // 이미지 경로 url/너비/높이
    const src = `./cat-551554_640.jpg/${width}/250`;
