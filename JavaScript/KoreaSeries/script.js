...
document.getElementById('predict-button').addEventListener('click', function(){
  ...
  while (gamesPlayed < maxGames && results['LG'] < 4 && results['KT'] < 4) {
    ...
    // 0.5 미만일 경우 LG승리, 그렇지 않으면 KT의 승리로 처리
...
function displayResults(results, gameOutcomes) {
  // 현재 시리즈의 결과를 문자열로 작성
  let resultsText = `현재 시리즈 결과: LG ${results['LG']} : KT ${results['KT']}`;
  ...
  document.getElementById('predict-result').innerHTML = resultsText
}