...
//? 1. 함수 선언식(Funtion Declaration)
// : 가장 기본적인 함수 정의 방법
// : funtion 키워드로 시작, 함수의 이름이 필수적으로 명시
...
//? 2. 함수 표현식(Funtion Expresstion)
// : 함수를 변수에 할당하는 방식
// : 함수에 이름을 붙일 수도 있고, 붙이지 않을 수도 있다.
// : 호이스팅 안됨 (정의된 후에만 호출 가능)
...
//? 3. 화살표 함수(Arrow Funtion)
// : ES6에서 도입된 함수 정의 방식
// : 기존의 funtion 키워드를 화살표로 대체하여, 간결한 함수를 작성
// : 항상 익명함수로 사용
...
log(); // 인수를 넘기지 않으면 - 출력 : undefined
log(1); // 인수를 하나 넘기면 - 출력 : 1
log(1, 2); // 인수를 두 개 이상 넘기면 - 출력 : 1 (지정된 수의 파라미터 이상의 인수는 읽히지 않는다.)
...
function noReturnValue () {
  console.log('This funtion dose not return anything.');
}
...
let sangHyun = 'SangHyun'; // 전역 변수
function ash () {
  let job = 'Developer'; // 지역 변수, 밖으로 나오면 인식안됨
  return sangHyun + job;
}
