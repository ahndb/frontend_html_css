...
//? substring 사용법
const redTextSubstring = str.substring(4, 17)
console.log(redTextSubstring); // substring(시작위치-포함O, 종료위치-포함X)
...
// 자바스크립트에서는 월의 시작이0, 8월은 7로 표시
function getQuarter(mouth) {
  // Math.ceil함수는 올림처리를 하여 분기를 계산
  // 1, 2, 3(1분기) 4,5,6(2분기), 7,8,9(3분기) 10,11,12(4분기)
  return Math.ceil(mouth / 3);
}
// 2022년 8월 20일에 해당하는 Date rorcp todtjd
const date = new Date(2022, 7, 20)

// getMouth 메소드를 사용하여 현재 월을 가져옴
// 결과값에 1을 더해 실제 월로 변환
...
// : 인접한 2개의 레코드를 비교해서 크기가 순서대로 되어있지 않으면 서로 교환
...
let uniqueFilterArr = combinedArr.filter((value, index, self) => {
  ...
  // 특정 요소나 문자열의 첫 번째 위치를 찾는데 사용하는 메소드
  return self.indexOf(value) === index;
});

console.log(uniqueFilterArr);
